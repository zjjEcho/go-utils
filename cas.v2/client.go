...
func requestURL(r *http.Request) (*url.URL, error) {
	u, err := url.Parse("/") // custom callback path by luohongyu
	// u, err := url.Parse(r.URL.String()) // original path
	if err != nil {
		return nil, err
	}
...
func (c *Client) RedirectToLogin(w http.ResponseWriter, r *http.Request, code int) {
	...
	//http.Redirect(w, r, u, http.StatusFound) // original 302
	// http.Redirect(w, r, u, http.StatusUnauthorized) // custom by luohongyu 401
	http.Redirect(w, r, u, code) // custom by luohongyu 401
}
...
func getCookie(w http.ResponseWriter, r *http.Request) *http.Cookie {
	...
	if err != nil {
		...
		c = &http.Cookie{
			...
			// MaxAge:   600,
			HttpOnly: false,
			Path:     "/", // ensure the client always use the same cookie(not the same path)
		}
